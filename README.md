...
本立體視覺資料集採用三目相機的立體相機系統，其架構主要延續由Sung-Yao Lan所提出的設計 L_Tri_PSMN。使用的攝影機為 logitech C310，並如下圖所示，左右鏡頭與上下鏡頭間的光心距離皆設為 7.5 公分。在以照相者觀點定左右的假設下，我們將左側攝影機視為基準視角，並透過空間座標轉換的方式，將由 LiDAR 相機所推算出的深度資訊對齊至基準相機的座標架構之中，如此所得再經過轉換成視差圖後，置於名為label的資料夾。   
...
資料集ground truth分為兩種方式來建立   
1: 7公尺以內使用Intel RealSense Lidar Camera L515   
2: 7公尺以上使用:L_Tri_PSMN預測搭配雷射測距儀驗證其準確度，並加上些許人工修正   
...
以下則展示使用L_Tri_PSMN預測搭配雷射測距範例，左圖為RGB圖、右圖為Ground Truth  。  
...
Reference  
***Liao, Zhan-Hao. (2025) "A real world scene dataset containing horizontal lines from a UAV perspective using a trinocular disparity camera", Master dissertation, National Chung Hsing University, Taiwan.***   
